...
\allsectionsfont{\centering \normalfont\scshape\vspace*{0pt}} % Make all sections centered, the default font and small caps
...
\fancyhead[L]{Christian Schlecht} % No page header - if you want one, create it in the same way as the footers below
\fancyhead[C]{cz17a}
\fancyhead[R]{\today}
\fancyfoot[L]{Lastenheft} % Empty left footer
...
\hypersetup {
    ...
    urlbordercolor=red,        % color of external links
    urlcolor=red
}
...
\author{Christian Schlecht} % Your name
...
\subsubsection{An- und Abmelden}
%\begin{longtable}{| m{2cm} | m{10cm} | m{2cm} |}
%    \hline
 %   \hypertarget{0101}FN01-01 & Übergabe eines Pfades zum RDF-File an den \texttt{jekyll build} Command (siehe Design) und/oder Suche in einem Default-Verzeichnis & Muss \\  \hline 
 %   \hypertarget{0102}FN01-02 & Unterstützter Dateityp: Turtle & Muss \\  \hline
%\end{longtable}
\begin{description}
...
\subsubsection{Quiz}
\paragraph{Rahmen} \label{par:Rahmen}
\begin{description}
...
\end{description}
\paragraph{Fragencharakteristik}
\begin{description}
...
(siehe Kapitel \ref{par:Rahmen} auf Seite \pageref{par:Rahmen}) Am Ende einer Runde soll eine Übersicht über Punktestände, anonymisierte Platzierungen und (evtl.) Fragenübersicht angezeigt werden.
\end{description}
...
\end{description}
\section{Nicht-funktionale Anwendungen}
\subsection{Muss-Ziele}
