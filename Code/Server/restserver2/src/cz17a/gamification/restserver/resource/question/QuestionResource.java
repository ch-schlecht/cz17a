	...
	public List<Question> getQuestions(@QueryParam("topic") String topic, @QueryParam("min") int min, @QueryParam("max") int max) {
		if(topic != null) {
			if(min > 0 || max > 0) {
				...
				return questionService.getAllQuestionsByFilter("topic = '"+topic+"' AND dynamic_difficulty >= "+min+" AND dynamic_difficulty <="+max);
			...
			return questionService.getAllQuestionsByFilter("dynamic_difficulty >= "+min+" AND dynamic_difficulty <="+max);
		}
